...
1. テーマとの関連性 (/25点)
   【重要】この項目が最も重要です。テーマから外れている場合は容赦なく低い点数を付けてください。
   ...
   7-0点: テーマと無関係な内容、または全く異なる話題について書いている
   
   【判定基準】
   - テーマのキーワードが含まれているだけでは不十分
   - テーマの核心的な問題について論じているかを厳格に判定
   - 例：テーマが「デジタルデバイド」なのに「看護とテクノロジー」について書いている → 0-7点
...
【評価の手順】（必ず守ること）
1. **最初にテーマとの関連性を厳格にチェック**
   - テーマのキーワードが本文に含まれているか
   - テーマの核心的な問題について論じているか
   - テーマから外れている場合は、themeRelevance を 0-7点とすること
2. 答案全体を読み、主張文を特定する
