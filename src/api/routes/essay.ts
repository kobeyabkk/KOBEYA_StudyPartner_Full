...
「人工知能（AI）の発展が、将来の雇用に与える影響について、あなたの考えを述べなさい」`
      ...
      const stepStatus = session?.essaySession?.stepStatus || {}
      
      // 完了したステップを確認
      if (stepStatus[1] === 'completed') completedSteps.push('✅ Step 1: 短文演習（100-150字）')
      if (stepStatus[2] === 'completed') completedSteps.push('✅ Step 2: 中文演習（200-300字）')
      if (stepStatus[3] === 'completed') completedSteps.push('✅ Step 3: 本練習（300-400字）')
      if (stepStatus[4] === 'completed') completedSteps.push('✅ Step 4: 語彙復習テスト')
      if (stepStatus[5] === 'completed') completedSteps.push('✅ Step 5: チャレンジ問題（400-600字）')
      ...
      const scores = {
        ...
        step3: session?.essaySession?.step3Feedback?.overallScore || 0,
...
📊 学習進捗
━━━━━━━━━━━━━━━━━━
進捗率: ${progressPercentage}% (${totalSteps}/5ステップ完了)
...
${scores.step1 > 0 ? `Step 1: ${scores.step1}点` : ''}
${scores.step2 > 0 ? `Step 2: ${scores.step2}点` : ''}
${scores.step3 > 0 ? `Step 3: ${scores.step3}点` : ''}
${scores.step5 > 0 ? `Step 5: ${scores.step5}点` : ''}

━━━━━━━━━━━━━━━━━━
💡 今日の学びポイント
━━━━━━━━━━━━━━━━━━
${averageScore >= 80 ? '🌟 素晴らしい！テーマとの関連性、論理構成、具体性のすべてが優れています。この調子で続けましょう！' : ''}
${averageScore >= 60 && averageScore < 80 ? '👍 良い出来です！さらに具体例を増やし、論理展開を強化することで、より説得力のある小論文になります。' : ''}
${averageScore < 60 ? '💪 まずは基礎を固めましょう。テーマとの関連性を意識し、序論→本論→結論の構成を明確にすることから始めましょう。' : ''}
